...
export default function AddRestaurantForm(props) {
  ...
  const addRestaurant = () => {
    if (!restaurantName || !restaurantAddress || !restaurantDescription) {
      ...
      toastRef.current.show("El lugar tiene que tener almenos una foto");
        ...
        db.collection("restaurants")
          ...
          .catch(error => {
            ...
            toastRef.current.show(
              "Error al subir el lugar, intento más tarde"
            );
          });
      });
    }
  ...
  useEffect(() => {
    (async () => {
      ...
      if (statusPermissions !== "granted") {
        toastRef.current.show(
          "Tienes que aceptar los permisos de localizacion para crear un lugar.",
          3000
        );
      } else {
